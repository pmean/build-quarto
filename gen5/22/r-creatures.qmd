...
```{r}
#| label: r-creatures-38

fast_food_summary <- summary(fast_food_regression)
names(fast_food_summary)
```

The third item in this list is coefficients, and it is different from the coefficients item in the previous list.

```{r}
#| label: r-creatures-39

fast_food_summary$coefficients
```

This is a matrix with names for each row and for each column. If you wanted to produce a nice table summarizing the results of the regression model, customized to your specifications, here is where you would start.
