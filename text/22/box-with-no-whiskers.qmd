...
Here's a boxplot showing some artificial data.
...
For the second data set, a large proportion of values are piled up at zero.

```{r}
quantile(x2)
```

This means that there is no difference between the minimum value and the 25th percentile. They are both zero. So drawing a whisker from the 25th percentile down to the minimum value means drawing no whisker at all.
