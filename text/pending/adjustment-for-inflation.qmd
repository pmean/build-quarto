...
The file cpi.csv lists the year and the month (M01, M02, etc.). you need to convert the opening date to a similar format so we can merge.

```{r}
#| label: convert-cpi-dates

hp |>
  mutate(Period=str_sub(opening_date, 1, 2)) |>
  mutate(Period=paste0("M", Period)) |>
  mutate(Year=str_sub(opening_date, 7, 10)) |>
  mutate(Year=as.numeric(Year)) -> hp1
