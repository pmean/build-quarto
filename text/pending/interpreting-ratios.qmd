...
out01 <- round(log_statistics$log_los_mean, 1)
out02 <- round(2^log_statistics$log_los_mean, 1)
out03 <- round(untransformed_statistics$los_median, 1)
out04 <- round(untransformed_statistics$los_mean, 1)
```

You saw earlier that the mean for the log length of stay was `r out01`.  Since this used the base 2 logarithm, you can transform back to the original scale of measurement by raising 2 to the log mean power. This produces `r out02` on the original scale. 

This quantity does not match the arithmetic mean `r round(untransformed_statistics$los_mean, 1)` that you computed before the transformation.
...
It is also worth noting in our example that the geometric mean, `r out02`, is much closer to the median, `r out03` than the arithmetic mean, `r out04`, is. The geometric mean reduces the impact of outliers on the high end.
...
m1
out05 <- round(m1$estimate[1], 1)
out06 <- round(m1$estimate[2], 1)
out07 <- round(m1$estimate[1] - m1$estimate[2], 1)
out08 <- round(2^out07, 2)
out09 <- round(group_means$los_geometric_mean[1], 2)
out10 <- round(group_means$los_geometric_mean[2], 2)
```

The two means, `r out05` and `r out06` are the means on the log scale that we saw earlier. The difference between these two means is `r out07` on the log scale. If you back-transform this to the original scale you get `r out08` which is the ratio of the two geometric means, `r out09` and `r out10`.
